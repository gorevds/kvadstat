...
kvadstat/sources/* возвращает CollectResult, который складывается в общие
таблицы blocks/flats/snapshots с глобально-уникальными id.

  python -m bin.scan_dev --db data/kvadstat.db --developer "ГК ФСК"
  python -m bin.scan_dev --db data/kvadstat.db --developer "ПИК"
  python -m bin.scan_dev --db data/kvadstat.db --all           # все 10 параллельно
...
from kvadstat.sources.base import CollectResult, SourceError, build_rows
from kvadstat.store import apply_schema, record_scan_run, refresh_materialized, upsert
...
def _pik_collect_for_known_blocks(db_path: Path) -> CollectResult:
    ...
    """
    log = logging.getLogger("kvadstat.scan_dev")
...
# Имя каждого источника обязано быть в реестре kvadstat.developers — иначе
...
assert not _UNKNOWN_SOURCES, (
    f"источники вне реестра застройщиков kvadstat.developers: {_UNKNOWN_SOURCES}"
)
...
def run_sweep(
    ...
    # scan_dev стартует ИЗ kvadstat-scan.service через OnSuccess, т.е. этот refresh
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--db", default="data/kvadstat.db", type=Path)
    parser.add_argument(
        "--developer",
