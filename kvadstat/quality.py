...
# Доля отбракованных квартир (гео-гейт), выше которой день — 'partial'.
REJECT_PARTIAL_FRACTION = 0.2

# Доля снапшотов с обнулённой ценой, выше которой день — 'partial': скрытые
# цены легитимны в малых дозах (брони, «по запросу»), но когда NULL у
# большинства — это смена формата/единиц у API (цены в копейках и т.п.),
# и день не годится для ценовой аналитики.
NULLIFIED_PARTIAL_FRACTION = 0.5
...
class DataQualityStats:
    ...
    rejected_price  — легаси-счётчик (с 2026-06 невалидная цена больше не
                      выбрасывает лот — см. nullified_price); остаётся 0;
    nullified_price — квартир записано с price=NULL: цена скрыта (бронь,
                      «по запросу») или мусорная (вне вилки). Присутствие
                      лота сохранено, чтобы velocity не увидела ложную
                      «продажу». НЕ входит в total_rejected_flats —
                      скрытая цена легитимна и не должна давать 'partial';
    rejected_geo    — квартир выброшено из-за гео-невалидного ЖК;
    geo_bad_blocks  — самих ЖК выброшено по гео.
    """
    rejected_price: int = 0
    nullified_price: int = 0
    rejected_geo: int = 0
