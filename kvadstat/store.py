...
from kvadstat.velocity import build_velocity_tables
...
def _migrate_blocks(conn: sqlite3.Connection) -> None:
    ...
    # из kvadstat.klass (курируемый маппинг + дефолт застройщика + авто по цене).
    if "klass" not in existing:
        conn.execute("ALTER TABLE blocks ADD COLUMN klass TEXT")
    # Backfill для строк без city — зеркало kvadstat.geo.city_from_address.
...
def apply_schema(conn: sqlite3.Connection) -> None:
    ...
    _migrate_flats(conn)
    sql = files("kvadstat").joinpath("schema.sql").read_text(encoding="utf-8")
