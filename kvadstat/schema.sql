...
CREATE TABLE IF NOT EXISTS blocks (
    ...
    longitude          REAL,
    address            TEXT,
    city               TEXT,        -- код города: 'msk' | 'mo' | 'spb' | ... (см. kvadstat.geo)
    distance_km        REAL,        -- расстояние от центра ПРАВИЛЬНОГО города (по city)
    floors_max         INTEGER,     -- этажность здания
    klass              TEXT         -- класс ЖК: эконом|комфорт|бизнес|премиум|элит (см. kvadstat.klass)
);
...
-- объекты создаются ИЗ Python (kvadstat/store.py: _create_views / refresh_materialized)
