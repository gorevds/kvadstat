...
from kvadstat.developers import ID_NAMESPACE, namespaced_id
from kvadstat.sources import fsk
...
def test_to_global_id_rehashes_out_of_range_numeric(caplog):
    ...
    from kvadstat.developers import split_id
...
def test_fsk_collect_with_mocked_api(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.fsk.request_json", fake_request_json)
    result = fsk.collect()
...
def test_fsk_rows_apply_to_schema(monkeypatch):
    ...
    from kvadstat.store import upsert
    for bp in blocks:
...
from kvadstat.sources import donstroy  # noqa: E402
...
def test_donstroy_collect_aggregates_floors_total_per_block(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.donstroy.request_json",
...
from kvadstat.sources import a101  # noqa: E402
...
def test_a101_collect_aggregates_floors_max_per_block(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.a101.request_json",
                        lambda *a, **k: pages.pop(0))
    result = a101.collect()
...
def test_a101_collect_follows_next_pagination(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.a101.request_json", fake_request_json)
    result = a101.collect()
...
from kvadstat.sources import level  # noqa: E402
...
def test_level_collect_aggregates_floors_section_total_per_block(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.level.request_json",
                        lambda *a, **k: pages.pop(0))
...
from kvadstat.sources import absolut  # noqa: E402
...
def test_absolut_collect_cursor_pagination(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.absolut.request_json", fake_request_json)
    result = absolut.collect()
...
def test_granel_project_meta_detects_non_msk_by_coords(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.granel.request_json",
        lambda *a, **k: {"results": [
...
def test_granel_collect_follows_next_pagination(monkeypatch):
    ...
    def fake_request_json(session, method, url, **kw):
        ...
        return pages.pop(0)
    monkeypatch.setattr("kvadstat.sources.granel.request_json", fake_request_json)
...
def test_granel_collect_upgrades_next_http_to_https(monkeypatch):
    ...
    monkeypatch.setattr("kvadstat.sources.granel.request_json", fake)
    granel.collect()
...
from kvadstat.sources import ingrad  # noqa: E402
...
from kvadstat.sources import brusnika  # noqa: E402
...
from kvadstat.sources import pik as pik_src  # noqa: E402
