...
from kvadstat.mapping import to_flat_row, to_snapshot_row
from kvadstat.sources import pik as pik_source
from kvadstat.sources.base import build_rows
from kvadstat.store import apply_schema, refresh_materialized, upsert
...
def test_pik_source_build_rows_produces_same_promo_as_legacy():
    ...
    """
    item = json.loads((FIXTURES / "sample_flat.json").read_text("utf-8"))
    # Legacy path: kvadstat.mapping.to_snapshot_row
    legacy = to_snapshot_row(item, scan_date="2026-05-25", scan_ts="t")
    # New path: NormFlat → build_rows
    from kvadstat.sources.base import CollectResult, NormBlock
